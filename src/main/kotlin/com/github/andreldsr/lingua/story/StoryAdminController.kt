...
class StoryAdminController(private val storyService: StoryService) {
    ...
    @Operation(summary = "Delete story by id", tags = ["Stories"], security = [SecurityRequirement(name = "bearer-key")])
    fun delete(@PathVariable id: Long) = storyService.delete(id)

    @PostMapping
    @ResponseStatus(HttpStatus.CREATED)
    @Operation(summary = "Create new story", tags = ["Stories"], security = [SecurityRequirement(name = "bearer-key")])
